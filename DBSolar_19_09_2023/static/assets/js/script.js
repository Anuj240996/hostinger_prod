...
(function($) {
	...
	// Sidebar no longer uses .slimscroll (native .sidebar-scroll in CSS). Other widgets may still use .slimscroll.
	var $slimScrolls = $('.slimscroll');

	function resizeSidebarSlimScroll() {
		var $legacy = $('.sidebar .slimscroll');
		if ($legacy.length === 0) {
			return;
		}
		var wHeight = $(window).height();
		$legacy.height(wHeight);
		$('.sidebar .slimScrollDiv').height(wHeight);
	}

	/** Keep the opened row visible inside the sidebar scroll pane. */
	function scrollSidebarItemIntoView($li) {
		var $ss = $('.sidebar .sidebar-scroll');
		if (!$ss.length || !$li || !$li.length) {
			return;
		}
		try {
			var itemTop = $li.offset().top;
			var paneTop = $ss.offset().top;
			var paneH = $ss.outerHeight();
			var rel = itemTop - paneTop + $ss.scrollTop();
			var target = Math.max(0, rel - 24);
			if (itemTop < paneTop || itemTop > paneTop + paneH - 40) {
				$ss.scrollTop(target);
			}
		} catch (err) {
			/* ignore */
	...
	function expandSidebarActiveBranches() {
		...
		// Include li.menu (e.g. "Project Details") which uses the same submenu pattern as li.submenu.
		$root.find('li.submenu, li.menu').each(function() {
			var $li = $(this);
			...
			// Prefer direct child <ul> (robust if comments/whitespace sit between <a> and <ul>).
	...
	function init() {
		var $this = Sidemenu;
		// Delegated: survives late DOM changes and matches the real #sidebar-menu on the page.
		// Only treat <a> as a submenu toggle when its parent <li> has a direct child <ul>.
		...
		$(document).off('click.sidemenu', '#sidebar-menu a').on('click.sidemenu', '#sidebar-menu a', function(e) {
			var $a = $(this);
			var $ownLi = $a.closest('li').first();
			var $nextUl = $ownLi.children('ul').first();
			if (!$nextUl.length) {
				return;
			}
			e.preventDefault();
			...
			if (!$a.hasClass('subdrop')) {
				$parentUl.children('li').each(function() {
					...
					var $peerSub = $peerLi.children('ul').first();
					if ($peerSub.length) {
						$peerSub.stop(true, true).hide();
						$peerA.removeClass('subdrop');
					}
				});
				// Instant show (not slideDown): inside overflow:auto sidebar, slide animations often collapse or clip.
				$nextUl.stop(true, true).css('display', 'block').show();
				$a.addClass('subdrop');
				scrollSidebarItemIntoView($ownLi);
			} else {
				$a.removeClass('subdrop');
				$nextUl.stop(true, true).hide();
			}
		});
	...
	// Sidebar Initiate
	init();
	// Re-run after full page load so nested `a.active` from server-rendered markup reliably expands
	// staff/vendor Dashboard (and other) submenus on first paint.
    ...
    /** HERAMB letterhead block — centered row; logoUrl should be absolute (origin + /static/...) */
    ...
    function attachGlobalDatatableTools(table) {
        ...
        /* No length dropdown (e.g. dt-no-pagination) — attach column/refresh tools next to search */
        if (!$lengthBox.length) {
            $lengthBox = $wrapper.find('.dataTables_filter');
        }
    ...
    /** Firereport / pages that re-init DataTable can call this to attach length + columns + refresh tools. */
    ...
    if ($('.datatable').length > 0 && $.fn.DataTable) {
        $('table.datatable').each(function() {
            ...
            // dt-compact-cols: let browser + CSS width:max-content size columns (no stretch-to-fill).
            // dt-no-pagination: show all rows; no page controls or "Show N rows" dropdown.
            var compactCols = $tbl.hasClass('dt-compact-cols');
            var noPagination = $tbl.hasClass('dt-no-pagination');
            /* data-default-order='[[0,"desc"]]' or '[]' (preserve server/HTML row order) */
            var dtOrder = [[0, 'asc']];
            var orderAttr = $tbl.attr('data-default-order');
            if (orderAttr !== undefined && String(orderAttr).trim() !== '') {
                try {
                    var parsedOrder = JSON.parse(orderAttr);
                    if (Array.isArray(parsedOrder)) {
                        dtOrder = parsedOrder;
                    }
                } catch (orderErr) { /* keep default */ }
            }
            var dt = $.fn.dataTable.isDataTable(this) ? $tbl.DataTable() : $tbl.DataTable({
                ...
                paging: !noPagination,
                lengthChange: !noPagination,
                pageLength: noPagination ? -1 : 10,
