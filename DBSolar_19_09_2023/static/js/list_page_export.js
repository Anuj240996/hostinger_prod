...
(function (global) {
    ...
    function arrayBufferToBase64(buffer) {
        ...
        return typeof global.btoa !== 'undefined' ? global.btoa(binary) : '';
    }

    /** Noto Sans includes U+20B9 (₹); Helvetica in jsPDF does not. */
    var PDF_NOTO_SANS_REGULAR_URL = 'https://cdn.jsdelivr.net/gh/googlefonts/noto-fonts@main/hinted/ttf/NotoSans/NotoSans-Regular.ttf';
    var PDF_NOTO_SANS_BOLD_URL = 'https://cdn.jsdelivr.net/gh/googlefonts/noto-fonts@main/hinted/ttf/NotoSans/NotoSans-Bold.ttf';

    /**
     * Embeds Noto Sans normal + bold into jsPDF so autoTable can render ₹ and other unicode.
     * Letterhead/caption keep Helvetica — they call setFont('helvetica') before drawing.
     * @param {*} pdf jsPDF instance
     * @param {function(boolean)} cb called with true on success
     */
    function embedNotoSansForPdf(pdf, cb) {
        ...
        Promise.all([
            fetch(PDF_NOTO_SANS_REGULAR_URL).then(function (r) {
                if (!r.ok) {
                    throw new Error('regular');
                }
                return r.arrayBuffer();
            }),
            fetch(PDF_NOTO_SANS_BOLD_URL).then(function (r) {
                if (!r.ok) {
                    throw new Error('bold');
                }
                return r.arrayBuffer();
            })
        ]).then(function (bufs) {
            pdf.addFileToVFS('NotoSans-Regular.ttf', arrayBufferToBase64(bufs[0]));
            pdf.addFont('NotoSans-Regular.ttf', 'NotoSans', 'normal');
            pdf.addFileToVFS('NotoSans-Bold.ttf', arrayBufferToBase64(bufs[1]));
    ...
    /** When unicode font fails to load, replace ₹ prefix with Rs. for Final Amount column(s). */
    function sanitizePdfFinalAmountColumnForAsciiFallback(headers, body) {
        var idxs = [];
        var hi;
        for (hi = 0; hi < headers.length; hi++) {
            if ((headers[hi] || '').trim().toLowerCase() === 'final amount') {
                idxs.push(hi);
            }
        }
        if (!idxs.length) {
            return body.map(function (r) { return r.slice(); });
        }
        return body.map(function (row) {
            var copy = row.slice();
            idxs.forEach(function (ci) {
                var cell = String(copy[ci] || '');
                copy[ci] = cell.replace(/^\u20B9\s*/, 'Rs. ');
            });
            return copy;
        });
    }
    ...
    /** @param {string} [logoSrcOverride] data URL or absolute URL; embed data URL for print so the logo is ready before print() */
    function listPdfPrintLetterheadInnerHtml(logoSrcOverride) {
        var logoSrc = logoSrcOverride || listPdfLogoAbsoluteUrl();
        ...
        function buildExportMatrix() {
            ...
            var body = rowIdxs.map(function (rowIdx, displayIdx) {
                return cols.map(function (colIndex) {
                    ...
                    /* Prefer data-export when set (amounts may use ₹ if pdfEmbedUnicodeFont is enabled for PDF). */
                    var cellNode = table.cell(rowIdx, colIndex).node();
                    if (cellNode && cellNode.getAttribute) {
                        var exportOverride = cellNode.getAttribute('data-export');
                        if (exportOverride !== null && String(exportOverride).trim() !== '') {
                            return String(exportOverride).trim();
                        }
                    }
...
//                    return clean;
                    ...
                    var allowNumberedListSplit = /serial|stock|description|item/.test(headerLower);
                    var allowQuantityUnitSplit = /qty|quantity|stock/.test(headerLower);

                    /* ✅ Handle numbered lists only for stock/serial-like columns */
                    if (allowNumberedListSplit && /\d+[.)]/.test(clean)) {
                        clean = clean
                            .replace(/\s*(\d+[.)])\s*/g, '\n$1 ')
                            .replace(/^\n/, '');
                    }

                    /* ✅ Handle quantity units only for qty/stock-like columns */
                    else if (allowQuantityUnitSplit && /(Nos|Ltr|Bunch)/i.test(clean)) {
                        clean = clean
...
//            var y = cell.y + padT + lineStepName * 0.85;
    ...
    function printWhenReady(winObj) {
        if (!winObj || !winObj.document) {
            return;
        }
     ...
     * @param {boolean} [cfg.pdfEmbedUnicodeFont] Fetch/embed Noto Sans so ₹ (U+20B9) renders in the PDF table; falls back to Rs. if load fails
     */
    function register(cfg) {
        ...
        function runPdf(logoDataUrl) {
            ...
            function drawPdfTableAndSave(tableBody) {
                autoOpts.body = tableBody;
            ...
            if (cfg.pdfEmbedUnicodeFont) {
                embedNotoSansForPdf(pdf, function (ok) {
                    if (ok) {
                        autoOpts.styles.font = 'NotoSans';
                        autoOpts.styles.fontStyle = 'normal';
                        autoOpts.bodyStyles.font = 'NotoSans';
                        autoOpts.headStyles.font = 'NotoSans';
                        drawPdfTableAndSave(exp.body);
                    } else {
                        drawPdfTableAndSave(sanitizePdfFinalAmountColumnForAsciiFallback(exp.headers, exp.body));
                    }
                });
            } else {
                drawPdfTableAndSave(exp.body);
