...
class PurchaseBill(models.Model):
    ...
    def get_eway(self):
        ...
        purchase_eway = PurchaseBillDetails.objects.get(billno=self)
        return purchase_eway.eway
...
class PurchaseItem(models.Model):
    ...
    perprice = models.DecimalField(max_digits=10, decimal_places=2, null=True)  # Store final amount
...
class FinalSale(models.Model):
    ...
    customer = models.ForeignKey(Customer, on_delete=models.CASCADE, related_name='final_sales', default=0)
...
class ReturnSale(models.Model):
    billno = models.AutoField(primary_key=True)
    time = models.DateTimeField(null=True)
    customer = models.ForeignKey(Customer, on_delete=models.CASCADE, related_name='return_sales', default=0)
    total_amount = models.DecimalField(max_digits=12, decimal_places=2, null=True, blank=True)
    is_deleted = models.BooleanField(default=False)
    update_time = models.DateTimeField(null=True)
...
class SaleBill(models.Model):
    billno = models.AutoField(primary_key=True)
    time = models.DateTimeField(null=True)
    Cust_id = models.ForeignKey(Customer, on_delete=models.CASCADE, related_name='salescustomer')
