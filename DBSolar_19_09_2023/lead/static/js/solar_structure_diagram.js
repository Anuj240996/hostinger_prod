    ...
    function buildSolarPanel3DGroup(panelW, panelLen, panelThick) {
        ...
        // Keep exported/front-view modules visually portrait (vertical rectangle).
        var faceW = panelW * 0.38;
        var faceL = panelLen * 4.08;
    ...
    function renderSolarStructureDiagram(container, opts) {
        ...
        drawAllPanelCells(function (t0, t1, u0, u1, label, row, visual) {
            ...
            var h = yBot - yTop;
            if (w < 4 || h < 4) return;
            svg += svgSolarPanelRect(xL, yTop, w, h);
            if (panelCount <= 12) {
                svg += '<text x="' + ((xL + xR) / 2) + '" y="' + ((yTop + yBot) / 2 + 4) + '" text-anchor="middle" font-size="' +
