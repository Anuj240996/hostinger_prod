    ...
    function buildSolarStructureLayout(opts) {
        ...
        // Full-span main rafters = 1 per leg column. Walkway's +2 are short gap members only.
    ...
    function addSolarWalkwayAndLadder(scene, layout, xAtLeg, roofPoint, alignMemberAlong, depth, frontTopY, backTopY, foundationH) {
        ...
        // Keep a usable gap so the walkway reads clearly between rows.
        if (t1 - t0 < 0.06) {
            var mid = (t0 + t1) / 2;
            t0 = Math.max(0.05, mid - 0.06);
            t1 = Math.min(0.95, mid + 0.06);
        }
        var midT = (t0 + t1) / 2;
        var roofMid = roofPoint(midT, 0.5);
        // Below solar panels (panelLift ~0.11), above main purlin (+0.05).
        var deckY = roofMid.y + 0.055;
        var pL = roofPoint(midT, 0);
        var pR = roofPoint(midT, 1);
        var deckW = Math.max(Math.abs(pR.x - pL.x) * 0.98, 0.5);
        var deckD = Math.max((t1 - t0) * depth * 0.98, 0.22);
        var zMid = midT * depth;
        var z0 = t0 * depth;
        var z1 = t1 * depth;
        ...
        // Solid walkway plate (highly visible) + grate bars on top.
        var plateMat = new THREE.MeshLambertMaterial({
            color: 0x9ca3af,
            transparent: true,
            opacity: 0.92
        });
        var plate = new THREE.Mesh(new THREE.BoxGeometry(deckW, 0.045, deckD), plateMat);
        plate.position.set(0, deckY, zMid);
        scene.add(plate);

        var frameMat = new THREE.MeshLambertMaterial({ color: 0x1f2937 });
        var barMat = new THREE.MeshLambertMaterial({ color: 0x374151 });
        var ft = 0.04;
        [
            ...
            fm.position.set(f[0], deckY + 0.03, zMid + f[1]);
            scene.add(fm);
        });
        var nLong = Math.max(8, Math.round(deckW / 0.1));
        var li;
        for (li = 1; li < nLong; li++) {
            var gx = -deckW / 2 + (deckW * li) / nLong;
            var bar = new THREE.Mesh(new THREE.BoxGeometry(0.03, 0.028, deckD * 0.92), barMat);
            bar.position.set(gx, deckY + 0.035, zMid);
            scene.add(bar);
        }
        ...
        for (cxi = 1; cxi < nCross; cxi++) {
            ...
            var cbar = new THREE.Mesh(new THREE.BoxGeometry(deckW * 0.92, 0.024, 0.03), barMat);
            cbar.position.set(0, deckY + 0.038, zMid + gz);
            scene.add(cbar);
        }

        // 2 orange walkway rafters ONLY under the walkway (last leg columns) — short span in the gap.
        var walkRafterMat = new THREE.MeshLambertMaterial({ color: 0xea580c });
        var lastCols = layout.legCols <= 2
            ? [0, Math.max(0, layout.legCols - 1)]
            : [layout.legCols - 2, layout.legCols - 1];
        lastCols.forEach(function (ci) {
            var x = xAtLeg(ci);
            var y0 = roofPoint(t0, 0.5).y - 0.04;
            var y1 = roofPoint(t1, 0.5).y - 0.04;
            var a = new THREE.Vector3(x, y0, z0);
            var b = new THREE.Vector3(x, y1, z1);
            if (a.distanceTo(b) < 0.05) return;
            var mesh = new THREE.Mesh(new THREE.BoxGeometry(0.12, a.distanceTo(b), 0.12), walkRafterMat);
            alignMemberAlong(mesh, a, b, axisY);
            scene.add(mesh);
        });

        // 4 blue walkway purlins across the plate (under grate).
        ...
        for (wp = 0; wp < 4; wp++) {
            ...
            left.y = deckY - 0.015;
            right.y = deckY - 0.015;
            var pm = new THREE.Mesh(new THREE.BoxGeometry(left.distanceTo(right), 0.06, 0.08), walkPurlinMat);
        ...
        // Ladder attaches to the FRONT edge of the walkway deck (gap side toward FRONT), not a rafter.
        ...
        var halfW = 0.15;
        // Top on walkway front edge, near the right third of the deck.
        var topX = deckW * 0.28;
        var topZ = z0 + 0.02;
        var topY = deckY + 0.05;
        // Foot on ground in front of the walkway (toward FRONT / z=0).
        var botX = topX + 0.35;
        var botZ = Math.max(0.08, z0 - depth * 0.22);
