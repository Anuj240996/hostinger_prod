...
def build_main_dashboard_context(request):
    """Shared context for /index/ and CRM main dashboard (same logic and counts)."""
    error = None
    ...
    try:
        from apps.core.views import build_crm_dashboard_context
        crm_ctx = build_crm_dashboard_context(request)
        context['total_leads'] = crm_ctx.get('total_leads', 0)
        context['total_quotations'] = crm_ctx.get('total_quotations', 0)
    except Exception:
        pass
    return context
...
def index(request):
    if is_associate_staff(request.user) and not request.user.is_superuser:
        return redirect('customer-view_all')
    return render(request, 'dashboard/index.html', build_main_dashboard_context(request))
