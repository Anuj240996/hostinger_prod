...
CP_PM_TABLE_HIDDEN_MODULE_NAMES = frozenset({"product", "lead", "quotation"})

# Retired Lead / Quotation menu groups (removed from Control Panel UI).
CP_NAV_RETIRED_SECTIONS = frozenset({"Lead", "Quotation"})
CP_NAV_RETIRED_KEY_PREFIXES = ("staff.lead.", "staff.quotation.", "staff.crm.")
...
def control_panel_permissions(request):
    ...
    module_seed = [
        ...
        ("firereport", "Fire Report", 7),
        ("barcode", "Barcode", 8),
        ("services", "Services", 9),
    ]
    CPModule.objects.filter(name__in=("lead", "quotation")).update(is_active=False)
    ...
    for prefix in CP_NAV_RETIRED_KEY_PREFIXES:
        CPNavItem.objects.filter(key__startswith=prefix).update(is_active=False)
    CPNavItem.objects.filter(section__in=CP_NAV_RETIRED_SECTIONS).update(is_active=False)

    nav_items = list(
        CPNavItem.objects.filter(is_active=True).select_related("portal").order_by("portal__order", "section", "order")
    )
    nav_items = [ni for ni in nav_items if ni.section not in CP_NAV_RETIRED_SECTIONS]
    if selected_category == "vendor":
        nav_items = [
            ni
            for ni in nav_items
...
def initialize_modules_permissions(request):
    ...
    if request.method == 'POST':
        ...
        default_modules = [
            ...
            {'name': 'dashboard', 'display_name': 'Dashboard', 'order': 3},
            {'name': 'product', 'display_name': 'Product', 'order': 4},
            {'name': 'inventory', 'display_name': 'Inventory', 'order': 5},
            {'name': 'transactions', 'display_name': 'Transactions', 'order': 6},
            {'name': 'firereport', 'display_name': 'Fire Report', 'order': 7},
            {'name': 'barcode', 'display_name': 'Barcode', 'order': 8},
            {'name': 'services', 'display_name': 'Services', 'order': 9},
        ]
