...
def control_panel_home(request):
    ...
    # Get all users (employees, associates)
    ...
    for user in users:
        ...
        if hasattr(user, 'profile'):
            ...
            if 'Engineer' in dept or 'Administration' in dept or 'Finance' in dept or 'Stockist' in dept:
                employees.append(user)
            else:
                associates.append(user)
        else:
            # Default to employee if no profile
            employees.append(user)
    ...
    vendor_accounts = {
        va.vendor_id: va
        for va in VendorAccount.objects.select_related("vendor", "user").all()
    }
    for v in vendors:
        vendor_rows.append(
            {
...
def control_panel_permissions(request):
    ...
    vendor_accounts = (
        VendorAccount.objects.select_related("vendor", "user")
    ...
    selected_targets_raw = request.GET.get("targets") or ""
    if request.method == "POST":
        selected_targets = [t for t in request.POST.getlist("targets") if t.strip().isdigit()]
    else:
        selected_targets = [t for t in selected_targets_raw.split(",") if t.strip().isdigit()]
    ...
    if CPNavItem.objects.count() == 0 and portals:
    ...
    if request.method == "POST" and request.POST.get("do_save") == "1":
        ...
        # If admin selected submodules, infer portal access automatically (prevents no-access surprises).
        # Only infer portals from nav items, but don't force the category portal.
        inferred_portals = {ni.portal.name for ni in nav_items if ni.key in desired_nav_keys}
        desired_portals |= inferred_portals
