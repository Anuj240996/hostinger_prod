...
class QuotationForm(forms.ModelForm):
    ...
    class Meta:
        ...
        fields = [
            ...
            'title',  # NEW: Add title field
            'lead', 'survey',
            'consumer_address1', 'consumer_address2', 'employee_name', 'panel_qty', 'inverter_qty',
            ...
            'panel_companies', 'inverter_companies',
            'structure_type', 'structure_back_height_ft', 'structure_front_height_ft',
            'special_discount', 'discount_percentage', 'gst_5_percent', 'gst_18_percent',
            'other_items', 'structure_warranty',
            'terms_conditions',
            'dc_capacity', 'ac_capacity', 'system_na',  # NEW: Add proposed system fields
            'electricity_unit_rate',  # Add this line
    ...
    def __init__(self, *args, **kwargs):
        ...
        # Indian-formatted amounts + empty pricing fields: normalize POST before validation.
        # NOTE: Views use QuotationForm(request.POST, ...) so data is the *first positional* arg,
        # not kwargs['data'] — we must mutate args[0], otherwise this block never runs.
        new_args = list(args)
        data_kw = kwargs.get('data')
        if data_kw is None and new_args and hasattr(new_args[0], 'copy'):
            data_kw = new_args[0]
        if data_kw is not None and hasattr(data_kw, 'copy'):
            indian_amount_keys = (
                'net_amount_input',
                'final_amount_input',
                'special_discount',
            ...
            for key in (
                ...
                vv = post_copy.get(key)
                if vv is not None and str(vv).strip() == '':
                    try:
                        del post_copy[key]
                    except (KeyError, TypeError):
                        pass
            # Pass normalized POST either as first positional *or* kwargs['data'], never both (TypeError).
            if new_args and len(new_args) > 0 and new_args[0] is data_kw:
                new_args[0] = post_copy
            else:
        ...
        # Model-bound GST amounts are computed on save; allow missing POST so defaults apply.
        if data_kw is not None:
            for fname in ('gst_5_percent', 'gst_18_percent'):
        ...
        # CRM Lead / Survey (same querysets as Lead CRM quotation form)
        if 'lead' in self.fields and 'survey' in self.fields:
            leads_qs = Lead.objects.filter(stage__in=['qualified', 'survey', 'quote'])
            if not leads_qs.exists():
                leads_qs = Lead.objects.all().order_by('-created')[:300]
            self.fields['lead'].queryset = leads_qs

            lead_ref = None
            if self.data and self.data.get('lead'):
                raw = (self.data.get('lead') or '').strip()
                if raw:
                    try:
                        lead_ref = Lead.objects.get(pk=raw)
                    except (Lead.DoesNotExist, ValueError):
                        pass
            if lead_ref is None and getattr(self.instance, 'lead_id', None):
                lead_ref = self.instance.lead
            if lead_ref is None and getattr(self.instance, 'pk', None):
                cn = (self.instance.consumer_name or '').strip()
                if cn:
                    lead_ref = Lead.objects.filter(name__iexact=cn).first()
            if lead_ref is None and self.initial.get('lead') is not None:
                il = self.initial.get('lead')
                if isinstance(il, Lead):
                    lead_ref = il
                elif il:
                    try:
                        lead_ref = Lead.objects.get(pk=il)
                    except (Lead.DoesNotExist, ValueError):
                        pass

            if lead_ref is not None:
                # Only surveys with a completion timestamp set (surveys_survey.completed_date)
                self.fields['survey'].queryset = Survey.objects.filter(
                    lead=lead_ref,
                    completed_date__isnull=False,
                ).order_by('-scheduled_date')
            else:
                # No lead yet — only show surveys after a lead is chosen (match UI)
        ...
        if 'lead' in self.fields:
            self.fields['lead'].label = 'Lead name'
            self.fields['lead'].empty_label = 'Select a lead'
            self.fields['lead'].widget.attrs.update({'class': 'form-control', 'id': 'id_crm_lead'})
        if 'survey' in self.fields:
            self.fields['survey'].label = 'Survey'
            self.fields['survey'].empty_label = 'Select a survey (optional)'
            self.fields['survey'].widget.attrs.update({'class': 'form-control', 'id': 'id_crm_survey'})
        if 'consumer_mobile' in self.fields:
            self.fields['consumer_mobile'].widget.attrs.update({
                'maxlength': '10',
                'minlength': '10',
                'pattern': r'\d{10}',
                'inputmode': 'numeric',
    ...
    def clean(self):
        ...
        if lead and survey and survey.lead_id != lead.id:
            self.add_error('survey', 'Select a survey that belongs to the chosen lead.')
        # Lead is optional: consumer_name is filled from lead when selected, otherwise set in the view.
        return cleaned

    def clean_consumer_mobile(self):
        mobile = (self.cleaned_data.get('consumer_mobile') or '').strip()
        digits_only = ''.join(ch for ch in mobile if ch.isdigit())
        if len(digits_only) != 10:
            raise forms.ValidationError('Consumer mobile number must be exactly 10 digits.')
        return digits_only
